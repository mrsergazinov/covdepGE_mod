...
-   **Goal:** estimate the underlying graph $G$ from given $n$ i.d.d. observations $x_1, \dots , x_p$. 
...
$$
- Further for each factor $q_k$ of the variational distribution we assume individual spike-and-slab density:
\begin{align*}
...
- Deriving the ELBO based on the assumed variational distribution and taking partial derivatives with respect to the variational parameters we obtain the following CAVI updates:
\begin{align*}
...
-   Pros:
    -   Theoretically  and intuitevely plausible.
-  Cons:
...
- Overfitting to the data.
    - We see that the fase negatives go down as the number of clusters goes up.
