...
##
## pip_assgn: NULL OR numeric vector with entries in {1, ..., n};
## assignmnet of each observation to a cluster, where each element
## in th cluster starts with the same prior inclusion probability (pip).
##
...
cavi <- function(X, Z, D, y, hp_method, ssq, sbsq, pip, pip_assgn, nssq, nsbsq,
  ...
  if (is.null(ssq) | is.null(sbsq) | is.null(pip)) {
    ...
    # if pip_assgn has not been supplied, create the grid
    if (is.null(pip_assgn)) {

      # create the assignment of each observation to a cluster
      pip_assgn <- rep(1, n)
    }
  ...
  hp_func <- function(curr_cluster) {
    ...
    return(list(
      alpha = alpha, mu = mu, ssq_var = ssq_var, elbo = elbo, hp = hp
    ))
  }
  ...
  for (assgn in unique(pip_assgn)) {
    ...
    elbo <- elbo + cluster_out$elbo
    hp[[assgn]] <- cluster_out$hp
  }
